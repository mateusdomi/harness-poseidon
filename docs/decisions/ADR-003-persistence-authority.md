...
Na Fase 1, o histórico PostgreSQL foi promovido para `harness.schema_migrations`; `harness_poc` contém somente a fila experimental. O SQLite possui runner próprio sobre o dispatcher único e tabela `schema_migrations`. As migrations de fundação materializam Tenant, Organização, Projeto, usuário local, Inbox, Outbox e ledger com FKs, unicidades, índices e `version` crescente equivalentes, mas tipos nativos distintos (`TEXT`/ISO-8601 no SQLite; `char(26)`, `jsonb` e `timestamptz` no PostgreSQL).
