...
Atualizado em: 2026-07-19. Este checklist separa gates automatizados de homologação humana. Um item só fica verde com evidência reproduzível; criar o arquivo ou executar parte do fluxo não equivale a aceite.

## Release candidate técnico

| Critério | Estado | Evidência/gate |
|---|---|---|
| Build reproduzível, format, regressão backend/frontend | verde | `tools/backend/verify.sh`; 246 backend + 331 frontend esperados após F11-7 |
...
O agregador técnico é `tools/backend/verify-release-candidate.sh`. Ele não fabrica evidência visual nem converte execução automática em homologação.

## Itens ainda necessários para GNG-6/DoD global

| Critério | Estado | Condição de fechamento |
|---|---|---|
| E2E do frontend contra API real | pendente externo ao backend | executar specs Playwright existentes com `tools/backend/playwright.real-api.config.ts` em navegador disponível e registrar resultado |
| Acessibilidade WCAG/axe | pendente externo ao backend | executar `frontend/e2e/a11y.spec.ts` nos viewports/temas definidos e registrar zero critical/serious |
