...
As pendências técnicas desta evidência inicial (instalação/atualização verificável e Runner no pacote) foram fechadas em `F7-DESKTOP-LIFECYCLE.md`. Resta no GNG-4 externo a distribuição com Developer ID/notarização e o aceite em macOS limpo. Gate original: format sem mudanças; build Release zero warnings/erros; 211/211 (`Unit 116`, `Integration 53`, `Contract 28`, `Recovery 5`, `Architecture 6`, `Concurrency 3`).
