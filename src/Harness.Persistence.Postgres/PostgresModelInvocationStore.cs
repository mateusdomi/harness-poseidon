...
public sealed class PostgresModelInvocationStore(NpgsqlDataSource dataSource) : IModelInvocationStore
{
    ...
    public async Task<IReadOnlyList<ModelInvocationRecord>> GetProjectInvocationsAsync(
        string tenantId, string projectId, int limit, CancellationToken cancellationToken = default)
    {
        ...
        // Paridade semântica com o SQLite: as `limit` invocações MAIS RECENTES do projeto.
        await using var command = _dataSource.CreateCommand("""
            SELECT id, tenant_id, project_id, work_task_id, attempt_id,
                   provider, model, account_alias, input_tokens, output_tokens,
                   estimated_cost_usd, duration_ms, outcome, invoked_at
    ...
    private static ModelInvocationRecord Read(NpgsqlDataReader reader) => new(
