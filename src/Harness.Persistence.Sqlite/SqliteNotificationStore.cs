...
public sealed class SqliteNotificationStore(SqliteWriteDispatcher dispatcher) : INotificationStore
{
    ...
    private static async Task<SettingsRecord> UpdateSettingsCoreAsync(SqliteConnection connection, SettingsUpdateCommand command, CancellationToken token)
    {
        ...
        await using var update = connection.CreateCommand(); update.Transaction = transaction; update.CommandText = "UPDATE profile_settings SET theme=$theme,language=$language,notifications_enabled=$enabled,muted_categories_json=$muted,working_directory=$working,updated_at=$at WHERE tenant_id=$tenant AND profile_id=$profile AND id=$id;";
        Add(update, "$theme", theme); Add(update, "$language", language.Trim()); Add(update, "$enabled", enabled ? 1 : 0); Add(update, "$muted", JsonSerializer.Serialize(muted, JsonOptions)); AddNullable(update, "$working", working); Add(update, "$at", Store(command.OccurredAt)); Add(update, "$tenant", command.TenantId); Add(update, "$profile", command.ProfileId); Add(update, "$id", command.Id);
        ...
        return current with { Theme = theme, Language = language.Trim(), NotificationsEnabled = enabled, MutedCategories = muted, WorkingDirectory = working, UpdatedAt = command.OccurredAt };
    }
    ...
    private static SettingsRecord ReadSettings(SqliteDataReader r) => new(r.GetString(0), r.GetString(1), r.GetString(2), r.GetString(3), r.GetInt32(4) == 1, JsonSerializer.Deserialize<string[]>(r.GetString(5), JsonOptions) ?? [], r.IsDBNull(6) ? null : r.GetString(6), Parse(r.GetString(7)));
    ...
    private const string SettingsSelect = "SELECT id,profile_id,theme,language,notifications_enabled,muted_categories_json,working_directory,updated_at FROM profile_settings";
