...
-- historia, tarefa, bug, adr, documento, revisao, gate, incidente, chamado. Os cinco tipos
-- pré-playbook permanecem válidos (dados existentes e rotas atuais não quebram). SQLite não
-- altera CHECK: recria a tabela com o mesmo padrão da migration 0076.
CREATE TABLE work_tasks_playbook_types
(
    ...
    card_type TEXT NOT NULL DEFAULT 'agent_task'
        ...
        (
            card_type IN
            (
                -- tipos pré-playbook, preservados por compatibilidade de dados e API
                'feature', 'agent_task', 'human_gate', 'spike', 'decision',
                -- tipos canônicos do playbook (§3): o vocabulário do produto
...
INSERT INTO work_tasks_playbook_types
(
    id,
    tenant_id,
    project_id,
...
ALTER TABLE work_tasks_playbook_types RENAME TO work_tasks;
