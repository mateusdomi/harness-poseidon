...
public sealed class SqliteRunTargetStore(SqliteWriteDispatcher dispatcher) : IRunTargetStore
{
        ...
        foreach (var definition in command.Definitions)
        {
            ...
            await ExecuteAsync(connection, transaction,
                ...
                VALUES ($tenant,$id,$project,$fingerprint,$name,$kind,$url,$port,'stopped',$working,
                        $executable,$arguments,$environment,$at,$at)
    ...
    private static async Task<RunTargetLaunchRecord?> GetLaunchCoreAsync(SqliteConnection connection, string tenant, string id, CancellationToken token)
    {
        ...
        if (!await reader.ReadAsync(token)) return null; var target = ReadPublic(reader); return new(target, reader.GetString(9), reader.GetString(10), JsonSerializer.Deserialize<string[]>(reader.GetString(11), JsonOptions) ?? [], JsonSerializer.Deserialize<Dictionary<string, string>>(reader.GetString(12), JsonOptions) ?? new(StringComparer.Ordinal));
    }
    ...
    private const string SelectPublic = "SELECT id,project_id,name,kind,url,port,state,detected_at,last_check_at FROM run_targets";
    private const string SelectLaunch = "SELECT id,project_id,name,kind,url,port,state,detected_at,last_check_at,working_directory,executable,arguments_json,environment_json FROM run_targets";
