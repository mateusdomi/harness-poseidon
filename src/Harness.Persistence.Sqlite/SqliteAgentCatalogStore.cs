...
public sealed class SqliteAgentCatalogStore(SqliteWriteDispatcher dispatcher) : IAgentCatalogStore
{
    ...
    private static async Task ValidateDefinitionReferencesAsync(SqliteConnection connection, SqliteTransaction tx, string tenant, AgentDefinitionContent content, CancellationToken token) { await ValidateCatalogReferenceAsync(connection, tx, "teams", "team", "/api/v1/teams", tenant, content.Team, token); await ValidateCatalogReferenceAsync(connection, tx, "specialties", "specialty", "/api/v1/specialties", tenant, content.Specialty, token); foreach (var skillId in content.SkillIds.Distinct(StringComparer.Ordinal)) await ValidateGlobalComponentAsync(connection, tx, "skills", "skill", "/api/v1/skills", skillId, token); foreach (var toolId in content.ToolIds.Distinct(StringComparer.Ordinal)) await ValidateGlobalComponentAsync(connection, tx, "tools", "tool", "/api/v1/tools", toolId, token); string? accountProvider = null; if (content.PreferredAccountId is not null) { await using var account = connection.CreateCommand(); account.Transaction = tx; account.CommandText = "SELECT provider_id FROM provider_accounts WHERE tenant_id=$tenant AND id=$id;"; Add(account, "$tenant", tenant); Add(account, "$id", content.PreferredAccountId); accountProvider = (string?)await account.ExecuteScalarAsync(token) ?? throw new AgentDefinitionAdminException("Preferred account was not found."); } var referencedModels = (content.FallbackModelIds ?? []).AsEnumerable(); if (accountProvider is not null && content.DefaultModelId is not null) referencedModels = referencedModels.Prepend(content.DefaultModelId); foreach (var modelId in referencedModels.Distinct(StringComparer.Ordinal)) { await using var model = connection.CreateCommand(); model.Transaction = tx; model.CommandText = "SELECT provider_id FROM provider_models WHERE tenant_id=$tenant AND id=$id AND enabled=1;"; Add(model, "$tenant", tenant); Add(model, "$id", modelId); var provider = (string?)await model.ExecuteScalarAsync(token) ?? throw new AgentDefinitionCatalogMissingException("model", modelId, "/api/v1/models"); if (accountProvider is not null && provider != accountProvider) throw new AgentDefinitionAdminException("Preferred account and models must use the same provider."); } }
    // CAT-04/CAT-05: uma definição só pode referenciar times/especialidades que existam no catálogo
    // tenant-scoped. Valores semeados (migração 0050) já constam do catálogo, preservando
    // as definições existentes; um valor novo e inexistente é recusado com um problema ACIONÁVEL
    // (catálogo + referência + rota de criação), não uma mensagem opaca.
    private static async Task ValidateCatalogReferenceAsync(SqliteConnection connection, SqliteTransaction tx, string table, string catalog, string createRoute, string tenant, string? value, CancellationToken token) { var trimmed = value?.Trim(); if (string.IsNullOrEmpty(trimmed)) return; await using var command = connection.CreateCommand(); command.Transaction = tx; command.CommandText = $"SELECT EXISTS(SELECT 1 FROM {table} WHERE tenant_id=$tenant AND lower(name)=lower($name));"; Add(command, "$tenant", tenant); Add(command, "$name", trimmed); if (Convert.ToInt64(await command.ExecuteScalarAsync(token), CultureInfo.InvariantCulture) == 0) throw new AgentDefinitionCatalogMissingException(catalog, trimmed, createRoute); }
    // CAT-05: skills/tools são catálogos GLOBAIS (sem tenant): a definição referencia por id. Um id
    // ausente devolve o mesmo problema acionável "criar quando não encontrar".
    private static async Task ValidateGlobalComponentAsync(SqliteConnection connection, SqliteTransaction tx, string table, string catalog, string createRoute, string id, CancellationToken token) { await using var command = connection.CreateCommand(); command.Transaction = tx; command.CommandText = $"SELECT EXISTS(SELECT 1 FROM {table} WHERE id=$id);"; Add(command, "$id", id); if (Convert.ToInt64(await command.ExecuteScalarAsync(token), CultureInfo.InvariantCulture) == 0) throw new AgentDefinitionCatalogMissingException(catalog, id, createRoute); }
    private static AgentDefinitionContent ToContent(AgentDefinitionRecord value) => new(value.Key, value.Name, value.Role, value.Specialty, value.Description, value.DefaultModelId, value.SkillIds, value.ToolIds, value.Persona, value.Mission, value.OperatingPrinciples ?? [], value.Deliverables ?? [], value.QualityCriteria ?? [], value.CommunicationStyle, value.Limitations ?? [], value.Stacks ?? [], value.DefaultEffort, value.PreferredAccountId, value.FallbackModelIds ?? [], value.Team, value.ActorCritic, value.Risk,
        value.AllowedScopes, value.DeniedScopes, value.ActivationCriteria, value.NonActivationCriteria);
    ...
    private static AgentDefinitionRecord ReadDefinition(SqliteDataReader reader) => new(
        ...
        JsonSerializer.Deserialize<string[]>(reader.GetString(31), JsonOptions) ?? [],
        JsonSerializer.Deserialize<string[]>(reader.GetString(32), JsonOptions) ?? [],
        JsonSerializer.Deserialize<string[]>(reader.GetString(33), JsonOptions) ?? [],
        JsonSerializer.Deserialize<string[]>(reader.GetString(34), JsonOptions) ?? []);
