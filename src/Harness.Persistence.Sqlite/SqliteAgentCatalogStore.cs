...
public sealed class SqliteAgentCatalogStore(SqliteWriteDispatcher dispatcher) : IAgentCatalogStore
{
    ...
    private static void ValidateDefinition(AgentDefinitionContent content) { if (string.IsNullOrWhiteSpace(content.Key) || content.Key.Length > 100 || content.Key.Any(character => !(char.IsLower(character) || char.IsDigit(character) || character == '-')) || string.IsNullOrWhiteSpace(content.Name) || content.Name.Length > 200 || content.Role is not ("chief" or "specialist") || string.IsNullOrWhiteSpace(content.Description) || content.Description.Length > 4000 || content.SkillIds.Concat(content.ToolIds).Any(value => !UlidValue.TryParse(value, out _)) || content.DefaultModelId is not null && !UlidValue.TryParse(content.DefaultModelId, out _)) throw new AgentDefinitionAdminException("Definition content is invalid."); }
