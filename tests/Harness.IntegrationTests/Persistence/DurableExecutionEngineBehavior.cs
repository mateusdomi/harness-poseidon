...
[SuppressMessage(
    ...
    Justification = "The same scenario must execute unchanged against both persistence providers.")]
internal static class DurableExecutionEngineBehavior
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        var started = await engine.StartAsync(request, initial, cancellationToken);
        var replay = await engine.StartAsync(request, initial, cancellationToken);
        ...
        var heartbeatResult = await engine.RecordHeartbeatAsync(
            LeaseCommand(lease, initial.AddSeconds(1), "heartbeat:1"),
            cancellationToken);
        ...
        var checkpoint = new DurableCheckpointCommand(
            ...
            "checkpoint:1");
        var checkpointResult = await engine.CheckpointAsync(checkpoint, cancellationToken);
        var checkpointReplay = await engine.CheckpointAsync(checkpoint, cancellationToken);
        ...
        var staleHeartbeat = await engine.RecordHeartbeatAsync(
            ...
            cancellationToken);
        var completedSnapshot = await engine.GetAsync(TenantId, lease.ExecutionId, cancellationToken);
        ...
        AssertJsonEquivalent("{\"commit\":\"abc123\"}", completedSnapshot.LatestCheckpointJson);

        await AssertRetryAndReconciliationAsync(engine, initial.AddMinutes(1), cancellationToken);
        await AssertTimerSignalAndLifecycleAsync(engine, initial.AddMinutes(2), cancellationToken);
    }
