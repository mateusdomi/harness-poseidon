...
public sealed class ProviderCatalogApiTests
{
    ...
    public async Task TenantCatalogSupportsRoutingBudgetSyncEventsAndRestartWithoutPersistingSecrets()
    {
        ...
        var cookies = new CookieContainer(); string profileId; string providerId; string modelId; string budgetId;
        try
        {
    ...
    { for (var i = 0; i < 200; i++) { var value = await client.GetFromJsonAsync<EventStreamSnapshot>("/api/v1/event-streams/snapshot?stream=global", token); if (value is not null && value.Delta.Count(x => x.Type == "quota.updated") >= 2 && value.Delta.Any(x => x.Type == "audit.eventAppended")) return value; await Task.Delay(25, token); } throw new TimeoutException("Provider events were not dispatched."); }
