        ...
        try
        {
            await using (var app = CreateHost(database))
            {
                ...
                try
                {
                    ...
                    Assert.DoesNotContain(customDefinitionId, (await client.GetFromJsonAsync<AgentDefinitionPage>("/api/v1/agent-definitions?limit=50", timeout.Token))!.Items.Select(value => value.Id));
                    Assert.Contains(customDefinitionId, (await client.GetFromJsonAsync<AgentDefinitionPage>("/api/v1/agent-definitions?limit=50&includeArchived=true", timeout.Token))!.Items.Select(value => value.Id));
