...
public sealed class ProviderContractDriftTests
{
    ...
    public void OpenApiMatchesProviderFrontendContracts(string route, string schema, string marker, string type, string csv, bool patch)
    {
        ...
        var source = File.ReadAllText(Path.Combine(root, "frontend", "src", "api", "contracts", "providers.ts")); var start = source.IndexOf($"export const {marker}", StringComparison.Ordinal); var end = source.IndexOf($"export type {type}", start, StringComparison.Ordinal); Assert.True(start >= 0 && end > start); var block = source[start..end]; var frontendFields = route == "accounts" ? fields.Take(6) : fields; Assert.All(frontendFields, f => Assert.Contains($"{f}:", block, StringComparison.Ordinal));
